...
function OpenPromoteMenu(society, employee)
	...
	ESX.TriggerServerCallback('esx_society:'..ecallback, function(jf)
		...
		for i=1, #jf.grades, 1 do
			...
			elements[#elements+1] = {icon = "fas fa-user", title = gradeLabel, value = jf.grades[i].grade, selected = (employee[selected].grade == jf.grades[i].grade)}
		end
...
function OpenManageSalaryMenu(society)
	...
	ESX.TriggerServerCallback('esx_society:'..ecallback, function(job)
		local elements = {
