...
AddEventHandler('esx_ambulancejob:useItem', function(itemName)
	...
	if itemName == 'bandage' then
		...
		ESX.Streaming.RequestAnimDict(lib, function()
			...
			TriggerClientEvent('idolo_hospital:client:ResetLimbs', source)
			TriggerClientEvent('idolo_hospital:client:RemoveBleed', source)
...
function StartShakeBody()
	CreateThread(function()
		...
		while timer > 0 and isDead do
			...
			AddTextComponentSubstringPlayerName(('Press [~b~H~s~] to Shake your Body'))			
			EndTextCommandDisplayText(0.175, 0.850)

            if IsControlJustReleased(0, 74) then
                --ShakeBody()
				ClearPedTasksImmediately(GetPlayerPed(-1))
				ESX.ShowNotification('Tu a bouger ton corp.')
				break
			end
		end
	end)
...
function SendDistressSignal()
   ...
   ESX.TriggerServerCallback('gksphone:namenumber', function(Races)
       ...
       TriggerServerEvent('gksphone:jbmessage', name, Races[1].phone_number, 'DISTRESS SIGNAL', '', GPS, 'ambulance')
   end)
end
	...
	CreateThread(function()
		...
		while bleedoutTimer > 0 and isDead do
			...
			if not Config.EarlyRespawnFine then
				text = "HOLD [~r~E~w~] TO RESPAWN OR WAIT FOR EMS"
...
function RemoveItemsAfterRPDeath()
	...
	CreateThread(function()
		...
		ESX.TriggerServerCallback('esx_ambulancejob:removeItemsAfterRPDeath', function()
			...
			TriggerServerEvent('idolo_hospital:server:RequestBed')
			Wait(10)
...
RegisterCommand('forcerespawn', function(source)
	if isDead then
		TriggerServerEvent('esx_ambulancejob:payFine')
		RemoveItemsAfterRPDeath()
	else
		ESX.ShowNotification('Juste les personne mort peut faire ceci')
	end
end)
------SEAT---------
RegisterNetEvent('menu:emsv')
AddEventHandler('menu:emsv', function()
	local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
	if closestPlayer ~= -1 and closestDistance <= 3.0 then
		TriggerServerEvent('oliannseat:putInVehicle', GetPlayerServerId(closestPlayer))
	else
...
AddEventHandler('oliannseat:putInVehicle', function()
...
-----UNSEAT-----
RegisterNetEvent('menu:emso')
AddEventHandler('menu:emso', function()
	local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
	if closestPlayer ~= -1 and closestDistance <= 3.0 then
		TriggerServerEvent('oliannseat:OutVehicle', GetPlayerServerId(closestPlayer))
	else
		ESX.ShowNotification(_U('no_players_nearby'))
	end
end)
...
AddEventHandler('oliannseat:OutVehicle', function()
	local playerPed = PlayerPedId()
...
RegisterNetEvent('esx_ambualncejob:userStresstabs')
AddEventHandler('esx_ambualncejob:userStresstabs', function()
	exports['oliann_progbar']:Progressbar("eat_something", "Taking Stress Tabs..", 2000, false, true, {
			...
			print('qweqweqwe')
		end, "stress_tabs") -- Item name
end)

RegisterCommand('id', function(source, args)
	  ESX.ShowNotification('Paypal ID: '..GetPlayerServerId(NetworkGetEntityOwner(GetPlayerPed(-1))))
  end)
