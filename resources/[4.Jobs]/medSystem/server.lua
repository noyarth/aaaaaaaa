...
RegisterServerEvent('medSystem:getplayer')
AddEventHandler('medSystem:getplayer', function(player)
		TriggerClientEvent('medSystem:send', player)
end)
...
for k,v in pairs(Config.job) do
	...
	elseif v.police and xPlayer.job.name == 'police' then
		if args[1] ~= nil then
			...
			TriggerClientEvent('esx:showNotification', source, 'ID Incorrect!')
		end		
	elseif v.mechanic and xPlayer.job.name == 'mechanic' then
		...
		TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer.source, { type = 'error', text = 'Tu est pas ambulancier!', length = 5000, style = { ['background-color'] = '#fc0000', ['color'] = '#ffffff' } })
	end
