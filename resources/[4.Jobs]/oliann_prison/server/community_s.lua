...
ESX.RegisterCommand('ccomserv', 'admin', function(xPlayer, args, showError)
    if  args.id ~= nil then
        TriggerEvent('esx_communityservice:sendToCommunityService', args.id, args.actions)


    else
        TriggerClientEvent('chat:addMessage', source, _U('invalid_player_id_or_actions'))
    end
end, false, {help = _U('give_player_community'), validate = true, arguments = {
    {name = 'id',  help = _U('target_id'), type = 'number'},
...
ESX.RegisterCommand('comserv', 'admin', function(xPlayer, args, showError)
    if  GetPlayerName(args.id) ~= nil then
        TriggerEvent('esx_communityservice:endCommunityServiceCommand', args.id)

    else
        TriggerClientEvent('chat:addMessage', source, _U('invalid_player_id'))
    end
end, false, {help = _U('unjail_people'), validate = true, arguments = {
    {name = 'id',  help = _U('target_id'), type = 'number'}
}})
