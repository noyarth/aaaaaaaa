...
AddEventHandler('esx:playerLoaded', function(xPlayer, isNew, skin)
	...
	if Config.EnableHud then
		...
		local factionTpl = '<div>{{faction_label}}{{grade_label}}</div>'

		local gradefLabel = ESX.PlayerData.faction.grade_label ~= ESX.PlayerData.faction.label and ESX.PlayerData.faction.grade_label or ''
		if gradefLabel ~= '' then gradefLabel = ' - '..gradefLabel end

		ESX.UI.HUD.RegisterElement('faction', #ESX.PlayerData.accounts, 0, factionTpl, {
			faction_label = ESX.PlayerData.faction.label,
			grade_label = gradefLabel
		})
	end
...
if Config.EnableHud then
    local gradefLabel = Faction.grade_label ~= Faction.label and Faction.grade_label or ''
    if gradefLabel ~= '' then gradefLabel = ' - '..gradefLabel end
