...
ESX.RegisterCommand('setfaction', 'admin', function(xPlayer, args, showError)
	if ESX.DoesFactionExist(args.faction, args.grade) then
		...
		showError(TranslateCap('command_setfaction_invalid'))
	end
...
end, true, {help = TranslateCap('command_setfaction'), validate = true, arguments = {
	{name = 'playerId', help = TranslateCap('commandgeneric_playerid'), type = 'player'},
	{name = 'faction', help = TranslateCap('command_setfaction_faction'), type = 'string'},
	{name = 'grade', help = TranslateCap('command_setfaction_grade'), type = 'number'}
}})
