...
lib.callback.register('oliann_sac:AddStash', function(source, data)
    CreateStash(data)
end)
...
RegisterNetEvent('buybag', function(v)
	...
	if money.count >= data.price then
		...
		TriggerClientEvent('oliann_sac:Notify',source, 'success', data.label..' has been Bought')
	else
		TriggerClientEvent('oliann_sac:Notify',source, 'error', data.label..' cannt afford pal')
	end
end)
...
ESX.RegisterUsableItem("bag", function(source,item,data)
	...
	TriggerClientEvent('oliann_sac:inventory',source,metadata)
...
local entities = {}
RegisterNetEvent("oliann_sac:placeobject", function(data)
	local source = source
...
RegisterNetEvent("oliann_sac:removeplacement", function(data)
	local source = source
	local bag = NetworkGetEntityFromNetworkId(data.net)
	if DoesEntityExist(bag) then
		...
		Wait(500)
		DeleteEntity(bag)
		TriggerClientEvent('oliann_sac:removezone',-1,data)
		data.src = source
