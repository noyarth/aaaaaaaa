...
lib.callback.register('oliann_hunting:slaughter', function(source, animalNetId, entityType, hasCorns, amountOfMeatLeftToGive)
        ...
        if hasKnife or Config.allowToSlaughterWithoutKnif then 
            if DoesEntityExist(entity) then
                ...
                if hasCorns then
                    ...
                    if amount then
                        if lib.callback.await('oliann_hunting:showPrgressbar', _source, locale('collecting_horns'), 2) then
                ...
                if addNext then
                    ...
                    if amount then
                        if lib.callback.await('oliann_hunting:showPrgressbar', _source, locale('collecting_meat_and_leather'), 4) then
...
RegisterNetEvent('oliann_hunting:signalIllegalHunting')
AddEventHandler('oliann_hunting:signalIllegalHunting', function(coords)
    local xPlayers = ESX.GetExtendedPlayers("job", "police")
	for _, xPlayer in pairs(xPlayers) do
        TriggerClientEvent('oliann_hunting:drawOutlaw', xPlayer.source, coords)
        Config.outlaw.signalfunc(source, coords, xPlayer.source)
    end
    
end)

RegisterNetEvent('oliann_hunting:removeOldEntity')
AddEventHandler('oliann_hunting:removeOldEntity', function(animalNetID)
...
lib.callback.register('oliann_hunting:canCarry', function(source, animalNetID)
...
RegisterNetEvent('oliann_hunting:setAmountOfMeat')
AddEventHandler('oliann_hunting:setAmountOfMeat', function(animlaNetId, amount)
    local animal = NetworkGetEntityFromNetworkId(animlaNetId)
    Entity(animal).state.amountOfMeatLeftToGive = amount
end)

lib.callback.register('oliann_hunting:getAmountOfMeat', function(source, animlaNetId)
    local animal = NetworkGetEntityFromNetworkId(animlaNetId)
    return Entity(animal).state.amountOfMeatLeftToGive
end)

RegisterNetEvent('oliann_hunting:setAnimalCarried')
AddEventHandler('oliann_hunting:setAnimalCarried', function(animlaNetId, status)
    local animal = NetworkGetEntityFromNetworkId(animlaNetId)
    Entity(animal).state.carried = status
end)

RegisterNetEvent('oliann_hunting:isAnimalCarried')
AddEventHandler('oliann_hunting:isAnimalCarried', function(animlaNetId)
    local animal = NetworkGetEntityFromNetworkId(animlaNetId)
    return Entity(animal).state.carried
end)

RegisterNetEvent('oliann_hunting:setVehicleState')
AddEventHandler('oliann_hunting:setVehicleState', function(vehicleNetId, state)
