...
local function spawnCar()
	local hasMoney = lib.callback.await('oliann_hunting:removeMoney', false, renCost)
	if hasMoney then
	...
	function point:nearby()
		...
		if (self.currentDistance <= range) and (isCurrentCarRentedByThisPalyer) and (vehicle ~=0) then
			...
			if IsControlJustPressed(0, 38) then
				...
				TriggerServerEvent('oliann_hunting:deleteCarAndRedeem', NetworkGetNetworkIdFromEntity(vehicle), sum)
			end
		else
