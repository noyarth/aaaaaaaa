...
local function spawnVehicle(data, spawn, price)
    ...
    TriggerServerEvent('oliann_garages:SpawnVehicle', data.vehicle.model, data.vehicle.plate, vector3(spawn.x, spawn.y, spawn.z-1), type(spawn) == 'vector4' and spawn.w or spawn.h, price)
end
    ...
    exports.ox_target:addGlobalVehicle({
        {
            ...
            event = 'oliann_garages:StoreVehicle',
...
for k, v in pairs(Config.Garages) do
    ...
    if not Config.ox_target then
        exports['qtarget']:AddTargetModel({v.ped or Config.DefaultGaragePed}, {
            options = {
                {
                    event = "oliann_garages:GetOwnedVehicles",
    ...
    exports.ox_target:addModel(pedsArr, {
        {
            ...
            event = 'oliann_garages:GetImpoundedVehicles',
            canInteract = function(entity)
                hasChecked = false
...
AddStateBagChangeHandler('vehicleData', nil, function(bagName, key, value, _unused, replicated)
    ...
    TriggerServerEvent('oliann_garages:ChangeStored', value.plate)
    Entity(vehicle).state:set('vehicleData', nil, true)
end)

RegisterNetEvent('oliann_garages:GetImpoundedVehicles', function()
    local vehicles = lib.callback.await('oliann_garages:GetImpound', false, currentImpound.type)
    local options = {}

    if not vehicles or #vehicles == 0 then
        lib.registerContext({
            id = 'oliann_garages:ImpoundMenu',
        ...
        return lib.showContext('oliann_garages:ImpoundMenu')
    end
    ...
    for i = 1, #vehicles do
        ...
        options[i] = {
            title = vehicleTitle,
            event = 'oliann_garages:ImpoundVehicleMenu',
            arrow = true,
    ...
    lib.showContext('oliann_garages:ImpoundMenu')
end)

RegisterNetEvent('oliann_garages:GetOwnedVehicles', function()
    local vehicles = lib.callback.await('oliann_garages:GetVehicles', false, currentGarage.type, currentGarage.job)
    local options = {}

    if not vehicles then
        lib.registerContext({
            id = 'oliann_garages:GarageMenu',
        ...
        return lib.showContext('oliann_garages:GarageMenu')
    end
    ...
    for i = 1, #vehicles do
        ...
        options[i] = {
            title = vehicleTitle,
            event = stored and 'oliann_garages:VehicleMenu' or nil,
    ...
    lib.showContext('oliann_garages:GarageMenu')
end)

RegisterNetEvent('oliann_garages:ImpoundVehicleMenu', function(data)
    lib.registerContext({
        id = 'oliann_garages:ImpoundVehicleMenu',
        title = data.name,
        menu = 'oliann_garages:ImpoundMenu',
        options = {
            [Locale('take_out_vehicle_impound')] = {
                ...
                event = 'oliann_garages:RequestVehicle',
                args = {
                    vehicle = data.vehicle,
                    price = data.price,
                    type = 'impound'
    ...
    lib.showContext('oliann_garages:ImpoundVehicleMenu')
end)

RegisterNetEvent('oliann_garages:VehicleMenu', function(data)
    lib.registerContext({
        id = 'oliann_garages:VehicleMenu',
        title = data.name,
        menu = 'oliann_garages:GarageMenu',
        options = {
    ...
    lib.showContext('oliann_garages:VehicleMenu')
end)

RegisterNetEvent('oliann_garages:RequestVehicle', function(data)
    local spawn = nil
...
RegisterNetEvent('oliann_garages:StoreVehicle', function(target)
    local vehicle = target.entity
    local vehPlate = GetVehicleNumberPlateText(vehicle)
    local vehProps = lib.getVehicleProperties(vehicle)

    local doesOwn, isInvalid = lib.callback.await('oliann_garages:CheckOwnership', false, vehPlate, vehProps.model, currentGarage)
    if doesOwn then
       if isInvalid then return ESX.ShowNotification(Locale('garage_cant_store')) end
        TriggerServerEvent('oliann_garages:SaveVehicle', vehProps, vehPlate, VehToNet(vehicle), currentGarage.zone.name)
    else
