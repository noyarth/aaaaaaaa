...
RegisterNetEvent("eyetakehostage")
AddEventHandler("eyetakehostage", function()
    callTakeHostage()
end)
...
function callTakeHostage()
	...
	if not canTakeHostage then 
		drawNativeNotification("Tu a besoin d'un pistol avec des balles pour prendre en otage!", 7000)
	end
	...
	if not takeHostage.InProgress and canTakeHostage then			
		...
		if closestPlayer then
			...
			if targetSrc ~= -1 then
				...
				drawNativeNotification("Personne proche!", 7000)
			end
		else
			drawNativeNotification("Personne proche!", 7000)
		end
...
Citizen.CreateThread(function()
	while true do 
		if takeHostage.type == "agressor" then
			...
			drawNativeText("Appuie [G] pour lacher, [H] pour tuer")
