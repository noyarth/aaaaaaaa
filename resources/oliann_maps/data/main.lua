...
AddEventHandler("playerSpawned", function ()
	-- If not already ran
	-- print("Hotel_Room playerSpawned")
	if not Ran then
		local cpt = 0
		local founded = false
		...
		while cpt < 300 do
			...
			if Vdist(coords,centerHotel)< 75.0 then
				for k,v in pairs(stairHeight) do
					-- print("checking stairs"..tostring(k))
					if v > zmin and v<zmax then
						currentStair = k
						founded = true
						timer = cpt
						cpt = 3001
						-- print("Stairs Found : "..tostring(k))
					end
				end
			end
		end
		if founded then
			for k,v in pairs(stairIpl[currentStair]) do
				-- print("load : "..tostring(v))
				RequestIpl(v)
			end
			-- print("Load Hotel Finished in : "..tostring(timer*100)"s".." stair : "..tostring(currentStair).."loaded")
		end
