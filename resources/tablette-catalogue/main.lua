...
RegisterNetEvent('oliann:cata:setvisible')
AddEventHandler('oliann:cata:setvisible', function(visibility)
    Gui(visibility)
end)

RegisterCommand("catalogue", function(source, args, rawCommand)
    if guiEnabled then
        Gui(false)
    else
        Gui(true)
